...
$user = $this->users_model->get_user_by_its_id($item['user_id']);
$username = $user['username'];
}
echo "<div><span class='w3-text-theme w3-left'><b><a href='".site_url('users/me/'.$username)."'>".
$user['firstname']." ".$user['lastname']."</a></b></span></div><br>";

echo nl2br($item['contents']);
if($item['img_url'] !=NULL)
{
echo "<div style='max-width:80%' class='w3-container'>";
echo "<img class='w3-image' src='".base_url('assets/topics/'.$item['img_url'])."'/>";
echo "</div>";
}
...
<div class="w3-container">
  <?php
echo  '<div class="w3-container w3-padding w3-border w3-small w3-margin w3-round-large w3-light-gray">';
$user = $this->users_model->get_user_by_its_id($needed_comment['user_id']);
$username = $user['username'];
  echo "<div><span class='w3-text-theme w3-left'><b><a href='".site_url('users/me/'.$username)."'>".
  $user['firstname']." ".$user['lastname']."</a></b></span></div><br>";

    echo $needed_comment['content'];
    if($needed_comment['img_url'] !=NULL)
    {
    echo "<div style='max-width:80%' class='w3-container'>";
    echo "<img class='w3-image' class='' src='".base_url('assets/topics/'.$needed_comment['img_url'])."'/>";
    echo "</div>";
?>
  <br>
  <?php
  ...
  echo  '<div class="w3-container w3-padding w3-border w3-small w3-margin w3-round-large">';
$user = $this->users_model->get_user_by_its_id($item['user_id']);
$username = $user['username'];
  echo "<div><span class='w3-text-theme w3-left'><b><a href='".site_url('users/me/'.$username)."'>".
  $user['firstname']." ".$user['lastname']."</a></b></span></div><br>";
    ...
    {
    echo "<div style='max-width:80%' class='w3-container'>";
    echo "<img class='w3-image' class='' src='".base_url('assets/topics/'.$item['img_url'])."'/>";
    echo "</div>";
    }
    echo "<br><div class='w3-text-theme'>";
